...
class EnvValidator {
  ...
  private constructor() {
    // 1. Validate Client Environment Variables
    ...
    // 2. Validate Server Environment Variables (Only if running on the server)
    if (typeof window === "undefined") {
      const serverEnvToValidate = {
        SUPABASE_URL: process.env.SUPABASE_URL,
        SUPABASE_ANON_KEY: process.env.SUPABASE_ANON_KEY,
      };

      const serverParsed = serverEnvSchema.safeParse(serverEnvToValidate);
